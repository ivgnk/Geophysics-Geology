"""
Из представленной выше программы сделай программу по выделению аномалий
только по исходным данным (без тренда и шума)
"""

"""
МЕТОД А: Выделение аномалий по правилу 3σ
Только для ИСХОДНЫХ ДАННЫХ (без добавления шума и тренда)
Программа: снятие тренда полиномами 1-4 порядков + медианная фильтрация
"""
...
def plot_results(original_field: np.ndarray, X: np.ndarray, Y: np.ndarray,
    ...
    contour2 = ax2.contourf(x_km, y_km, best_result.residual, levels=50, cmap=cmap, alpha=0.9)
...
def main():
    ...
    print(f"   Диапазон поля: [{np.min(field):.1f}, {np.max(field):.1f}] мГал")
    ...
    plot_optimization_summary(results, n_top=15)
    ...
    with open('anomaly_detection_results.json', 'w', encoding='utf-8') as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)

    print("   Результаты сохранены в anomaly_detection_results.json")
