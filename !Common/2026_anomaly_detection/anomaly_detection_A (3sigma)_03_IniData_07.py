"""
Сделай полный вариант программы с исправлением: в final_results:
1) Все карты должны быть одного размера
2) цветовые шкалы: а) горизонтальные; б) под соответствующими диаграммами;
в) одного размера;
3) для третьей карты добавь цветовую шкалу
4) все шкалы должны иметь подпись величины и ее размерность, например dg, мГал
5) для последней карты внизу отдельно легенда (ниже цветовой шкалы ),
которая поясняет что такое белые точки, синие кружки и красная прерывистая линия
6) Подписи карт на верху должны быть полные, например, исходное гравитационное поле и т.д.

1) Раздели final_results на два отдельные файла:
final_results01 - первые две карты, final_results02 - третья карта
1.1) Сделай, чтобы цветовые шкалы по высоте были равны соответствующей карте
1.2) Сделай, чтобы цветовые шкалы были ближе к своей карте, чтобы подписи на шкале не налезали на соседнюю карту
1.3) dg, мГал сделай в две строки над цветовой шкалой
2) Сделай полный текст программы
"""
...
- Раздельная визуализация: три отдельных файла
- Унифицированное форматирование цветовых шкал (высота = высоте карты, ширина = 0.05)
"""
...
def main():
    ...
    print("\n6. Финальная визуализация (раздельная)...")
    best = results[0]
