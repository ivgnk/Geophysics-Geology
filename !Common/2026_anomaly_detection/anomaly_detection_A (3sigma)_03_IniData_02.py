"""
Переделай программу так, чтобы k (размер окна) менялся от 0.01 до 0.25 с логарифмическим шагом и
всего было 8 значений, включая 0.01 до 0.25
"""
...
def optimize_parameters(field: np.ndarray, X: np.ndarray, Y: np.ndarray,
    ...
    """
    Полный перебор параметров: полиномы 1-4 + медианная фильтрация с логарифмическим шагом
    """
    ...
    if median_k_values is None:
        ...
        median_k_values = generate_logspace_k_values(0.01, 0.25, 8)
    ...
    if verbose:
        ...
        print(f"  Медианный фильтр (k): {[f'{k:.5f}' for k in median_k_values]}")
    ...
    for poly_order in polynomial_orders:
        for median_k in median_k_values:
            ...
            try:
                ...
                if verbose:
                    print(f"  Ошибка для poly={poly_order}, k={median_k:.5f}: {e}")
                continue
    ...
    if verbose:
        ...
        for i in range(min(5, len(results))):
            ...
            print(f"   Параметры: полином={r.params['polynomial_order']}, "
                  f"k={r.params['median_k']:.5f} (размер={r.params['median_size']})")
...
def plot_results(original_field: np.ndarray, X: np.ndarray, Y: np.ndarray,
    ...
    ax2.set_title(f'После снятия тренда (с окантовкой)\n(полином={best_result.params["polynomial_order"]}, '
                  f'k={best_result.params["median_k"]:.5f})')
    ax2.set_xlabel('X, км')
    ax2.set_ylabel('Y, км')
    ax2.set_xlim(0, 5)
    ax2.set_ylim(0, 5)
    ...
    for i, poly in enumerate(poly_orders):
        for j, k in enumerate(k_values):
            for r in all_results:
                if r.params['polynomial_order'] == poly and abs(r.params['median_k'] - k) < 0.0001:
    ...
    ax4.set_xlabel('k (размер медианного фильтра) - логарифмическая шкала')
    ...
    # Настройка отображения k на логарифмической шкале
    ax4.set_xscale('log')
    ...
    im2 = ax5.imshow(n_detected_matrix, aspect='auto', cmap='viridis',
                     extent=[min(k_values), max(k_values), len(poly_orders), 0])
    ax5.set_xlabel('k (размер медианного фильтра) - логарифмическая шкала')
    ...
    • Медианный фильтр: k = {best_result.params['median_k']:.5f}
    ...
    plt.suptitle(f'{title}\nВыделение аномалий методом 3σ (логарифмический шаг k)', fontsize=14)
    plt.tight_layout()
...
def plot_optimization_summary(all_results: List[OptimizationResult], n_top: int = 15,
    ...
    ax.scatter(range(len(top_results)), k_values, s=100, alpha=0.6,
               label='k (медианный фильтр, лог. шкала)', marker='s', color='purple')
    ax.set_yscale('log')
    ax.set_xlabel('Ранг')
    ax.set_ylabel('Значение параметра (лог. шкала для k)')
    ax.set_title('Параметры лучших результатов (логарифмическая шкала)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    plt.suptitle('Анализ результатов оптимизации (логарифмический шаг k)', fontsize=14)
    plt.tight_layout()
...
def main():
    ...
    print(f"\n   Значения k (логарифмический шаг):")
    for i, k in enumerate(k_values, 1):
        print(f"      {i}. {k:.6f} (размер окна: {int(grid_size * k)}×{int(grid_size * k)} пикселей)")
    ...
    results = optimize_parameters(
        ...
        polynomial_orders=[1, 2, 3, 4],
        median_k_values=k_values,  # Используем логарифмические значения
        use_ransac=True,
        n_sigma=2.0,
        min_area_pixels=25,
        verbose=True
    ...
    print(
        f"   • k = {best.params['median_k']:.6f} (размер окна = {best.params['median_size']}×{best.params['median_size']})")
    ...
    print(f"\n✓ Анализ параметра k (логарифмическая шкала):")
    print(f"   • Малые k (0.01-0.03): сильное сглаживание, выделяются крупные аномалии")
    print(f"   • Средние k (0.03-0.10): оптимальный баланс")
    print(f"   • Большие k (0.10-0.25): слабое сглаживание, много мелких деталей")
