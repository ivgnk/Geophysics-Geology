"""
На рисунке parameter_dependencies 4 разных картинки,
можно получить одну результирующую по оценке качества?
Сначала предложи варианты на рассмотрение,
код пока не надо

Сделай в программе вариант 3 и вариант 7
"""
...
def evaluate_result_fast(detection: Dict, sources: List) -> Dict:
    ...
    detected_areas = []  # В упрощённой версии площади не вычисляются
    area_score = 0.5  # Значение по умолчанию
...
def plot_radar_chart(results: List, output_dir: str = "results") -> plt.Figure:
    """
    ВАРИАНТ 3: Радарная диаграмма (паутинная)
    Показывает чувствительность качества к каждому параметру
    ...
    # Берём топ-100 результатов для анализа
    ...
    # Вычисляем среднюю оценку для каждого значения параметра
    sensitivity = {}
    for param, values in param_ranges.items():
        sensitivity[param] = []
        for val in values:
            scores = []
            # Собираем оценки для всех результатов с данным значением параметра
            for r in top_results:
                param_value = r['params'][param]

                if val > 0:
                    # Для ненулевых значений параметра (k, n_sigma, min_area)
                    # Проверяем, находится ли значение в пределах 1% от val
                    if abs(param_value - val) < 0.01 * val:
                        scores.append(r['evaluation']['score'])
                else:
                    # Для нулевых значений (на случай, если они появятся)
                    if param_value == val:
                        scores.append(r['evaluation']['score'])
    ...
    for idx, (ax, param) in enumerate(zip(axes.flatten(), param_list)):
        ...
        # Вычисляем углы для каждой оси (равномерно по кругу)
...
def plot_parallel_coordinates(results: List, n_top: int = 200, output_dir: str = "results") -> plt.Figure:
    """
    ВАРИАНТ 7: Параллельные координаты
    Каждая комбинация параметров — ломаная линия, цвет = оценка качества
    """
    # Берём топ-N результатов для наглядности
    ...
    # Нормализация параметров для отображения на одной шкале
    ...
    # Добавление подписей значений на осях
...
def plot_parameter_dependencies(results: List, polynomial_orders: List[int],
    ...
    """Стандартная визуализация зависимости качества от параметров"""
