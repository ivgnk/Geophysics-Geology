"""
Сделай X и Y как и в основной программе, т.к. в все и контуры оказались
мелкими точками в левом верхнем углу карты.
Там же видимо дырки бинарной маски. Сделай полный текст всей программы
"""
...
ВИЗУАЛИЗАЦИЯ:
- final_results_part1a_original_*.png - исходное поле
- final_results_part1b_detrended_*.png - поле после снятия тренда  
- final_results_part2_anomalies_*.png - выделенные аномалии с легендой
- binary_mask_approximation_*.png - бинарная маска и аппроксимация (полигоны, эллипсы)
...
OPTIMIZATION_CONFIG = {
    ...
    'use_numba': 0,  # JIT-компиляция Numba (0 - отключена для совместимости)
    'use_cupy': 0,  # GPU-ускорение CuPy (0 - отключено)
...
def create_detailed_detection(residual: np.ndarray, X: np.ndarray, Y: np.ndarray,
    ...
    return {
        ...
        'centers': centers,
        'areas_km2': areas_km2,
        'mask': mask  # возвращаем бинарную маску для дополнительной визуализации
    }
...
def get_detailed_detection(residual: np.ndarray, X: np.ndarray, Y: np.ndarray,
                           n_sigma: float = 1.5, min_area_pixels: int = 15) -> Dict:
    """
    Получение детальной информации о выделенных аномалиях (полигоны, эллипсы, центры, площади)
    без визуализации.
    """
...
def plot_binary_mask_with_approximation(X: np.ndarray, Y: np.ndarray,
                                        mask: np.ndarray,
                                        polygons: List[Polygon],
                                        ellipses: List[Ellipse],
                                        title: str = "Бинарная маска и аппроксимация",
                                        output_dir: str = None) -> plt.Figure:
    """
    Визуализация бинарной маски в физических координатах (км) с наложенными аппроксимациями.

    Параметры:
    - X, Y: координатные сетки (в метрах)
    - mask: булев массив (True — аномалия, False — фон) в координатах сетки
    - polygons: список полигонов (красные контуры) в координатах (км)
    - ellipses: список эллипсов (синие пунктирные контуры) в координатах (км)
    - title: заголовок графика
    - output_dir: директория для сохранения (если указана)
    """
    ...
    # origin='lower' - правильная ориентация (Y возрастает снизу вверх)
    # extent = [xmin, xmax, ymin, ymax]
    ax.imshow(mask, cmap='gray', origin='lower',
              extent=[x_km.min(), x_km.max(), y_km.min(), y_km.max()],
              alpha=0.7)

    # Накладываем полигоны (красные)
    for polygon in polygons:
        ax.add_patch(polygon)

    # Накладываем эллипсы (синие пунктирные)
    ...
    # Легенда
    legend_elements = [
        Line2D([0], [0], color='gray', marker='s', markerfacecolor='gray',
               markersize=10, label='Аномалия (бинарная маска)', linestyle='None'),
        Line2D([0], [0], color='red', linewidth=2, label='Выпуклый полигон (≥12 вершин)'),
        Line2D([0], [0], color='blue', linewidth=2, linestyle='--',
               label='Аппроксимирующий эллипс')
    ]
    ...
    if output_dir:
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = os.path.join(output_dir, f"binary_mask_approximation_{timestamp}.png")
        fig.savefig(filepath, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"   Сохранено: {filepath}")
...
def main():
    ...
    # Получаем detailed_detection для дополнительной визуализации
    detailed_detection = get_detailed_detection(
        residual, X, Y,
        n_sigma=best['params']['n_sigma'],
        min_area_pixels=best['params']['min_area_pixels']
    )

    # Визуализация бинарной маски с аппроксимациями
    plot_binary_mask_with_approximation(
        X=X, Y=Y,
        mask=detailed_detection['mask'],
        polygons=detailed_detection['polygons'],
        ellipses=detailed_detection['ellipses'],
        title=f"Бинарная маска и аппроксимация (обнаружено {detailed_detection['num_anomalies']} аномалий)",
    ...
    print(f"   • binary_mask_approximation_*.png - бинарная маска и аппроксимация")
